...
def _condutores_circuito_unifilar(circuito, criterio=""):
    ...
    # Rev.121: three-way representado pela topologia física real.
    if tipo.startswith("ILUM"):
        ...
        if criterio == "LUZ_PARA_INTERRUPTOR_PARALELO_2":
            # Rev.121: 2 viajantes + 1 retorno no eletroduto que liga a
            # caixa da luminária ao segundo interruptor paralelo.
            return ["R", "R", "R"]
        if criterio == "INTERRUPTOR_PARA_LUZ_PARALELO_2":
            # Compatibilidade com rotas antigas da Rev.121.
            return ["R"]
        if criterio in {
...
def _desenhar_quadro_chamada_unifilar(
    ...
    """Rev.121: balão circular numerado com anti-colisão e leader preso ao eletroduto.
...
def _desenhar_tabela_legenda_condutos_unifilar(msp, registros, ambientes_geom):
    """Rev.121: LEGENDA DE FIAÇÃO gráfica, compacta e baseada na referência do usuário.
    ...
    # Fase 13.6 Rev.121 — dimensões automáticas por conteúdo.
    ...
    for idx, reg in enumerate(registros):
        ...
        # Fase 13.6 Rev.121 — cada grupo ocupa exatamente o espaço necessário
...
def _desenhar_identificacao_condutos_unifilar(msp, rotas_fisicas, circuitos, ambientes_geom):
    """Fase 13.6 Rev.121 — balões anti-colisão + legenda gráfica de fiação."""
    por_numero = {}
...
def gerar_cad_unifilar(
    ...
    try:
        ...
        for polilinha in polilinhas:
            ...
            # Fase 13.6 Rev.121 — a geometria do ambiente só pode ser
        ...
        # Fase 13.6 Rev.121 — chamadas numeradas ancoradas na geometria real; detalhes elétricos
        ...
        # Fase 13.6 Rev.121 — diagrama unifilar retirado do DXF.
        ...
        if auditoria_normativa_qdc.get(
            ...
            raise ValueError(
                "QDC bloqueado pela auditoria elétrica da Fase 13.6 Rev.121: "
                + detalhes_bloqueio
            )
