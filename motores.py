...
def processar_dxf(caminho_arquivo):
    ...
    # Recaptura correta das portas no escopo global
    portas = []
    for entity in msp:
        tipo = entity.dxftype()
    ...
    for polilinha in polilinhas:
        ...
        # 1. Distribuição de Luz e Interruptor Exato a 10cm da Porta Tangenciando a Parede
        if nome in dict_dados:
            ...
            if qtd_ilum > 0:
                ...
                # POSICIONAMENTO EXATO DO INTERRUPTOR (10cm da ponta do vão/soleira no lado oposto à dobradiça)
                ...
                if unique_portas and logical_walls:
                    ...
                    # Pega as duas extremidades do vão da porta (p1 e p2)
                    pt_porta_1, pt_porta_2 = p_porta['p1'], p_porta['p2']
                    
                    # Identifica qual extremidade está a dobradiça (mais próxima de uma das pontas da parede) e qual é o lado oposto (maçaneta)
                    ...
                    # A ponta do vão oposta à dobradiça é o ponto de referência onde mediremos os 10cm para a parte externa/parede
                    ponto_referencia_vão = pt_porta_2 if d1 < d2 else pt_porta_1
                    
                    # Afasta exatamente 0.10m (10cm) da ponta do vão ao longo da parede, tangenciando para dentro do ambiente com o vetor normal (nx, ny)
                    direcao_sinal = 1 if (ponto_referencia_vão[0] - mid_porta_x >= 0 and abs(w_vx) > 0.5) or (ponto_referencia_vão[1] - mid_porta_y >= 0 and abs(w_vy) > 0.5) else -1
                    
                    sw_x = ponto_referencia_vão[0] + (w_vx * 0.10 * direcao_sinal) + (nx * 0.12)
                    sw_y = ponto_referencia_vão[1] + (w_vy * 0.10 * direcao_sinal) + (ny * 0.12)
