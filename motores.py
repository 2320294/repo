...
def gerar_cad_unifilar(dxf_bytes, dados_editados, local_qdc):
    ...
    try:
        ...
        # 1. ENCONTRA SOLEIRAS COM PORTA ENCOSTADA (< 0.4m) - Sem limitação de corredores
        soleiras_com_porta = []
        ...
        # 2. PROCESSA CADA SOLEIRA VÁLIDA
        for item in soleiras_com_porta:
            ...
            if len(adjacentes) >= 2:
                # Faz divisa com dois ambientes: gera dois círculos (um em cada extremidade oposta, voltados para dentro)
                pontos_opostos = [ponto_oposto_1, ponto_oposto_2]
                for idx, poly in enumerate(adjacentes[:2]):
                    ...
                    if d_tot > 0:
                        dir_x, dir_y = (cx - p_op[0]) / d_tot, (cy - p_op[0]) / d_tot # Corrigido para dir_y
                ...
                # Não faz divisa com outro ambiente (único ambiente): gera apenas um círculo voltado para dentro
                poly = adjacentes[0]
                cx = sum([pt[0] for pt in poly]) / len(poly)
                cy = sum([pt[1] for pt in poly]) / len(poly)
                d_tot = math.hypot(cx - ponto_oposto_1[0], cy - ponto_oposto_1[1])
                if d_tot > 0:
                    dir_x, dir_y = (cx - ponto_oposto_1[0]) / d_tot, (cy - ponto_oposto_1[1]) / d_tot
