...
def processar_dxf(caminho_arquivo):
    ...
    for polilinha in polilinhas:
        ...
        # 1. Distribuição de Luz e Interruptor Ajustado (15cm a partir da extremidade do batente/vão oposta à dobradiça)
        if nome in dict_dados:
            ...
            if qtd_ilum > 0:
                ...
                # POSICIONAMENTO PRECISO DO INTERRUPTOR
                ...
                if unique_portas and logical_walls:
                    ...
                    # Ordena as pontas do vão para pegar exatamente a extremidade que encosta na batente/parede do lado oposto à dobradiça
                    # Usamos a menor coordenada ou a extremidade mais distante do centro da parede de apoio
                    if abs(w_vx) > 0.5: # Parede horizontal
                        ponto_batente_oposto = max([pt_porta_1, pt_porta_2], key=lambda pt: pt[0]) if pt_porta_1[0] != pt_porta_2[0] else pt_porta_1
                        # Se precisar inverter o lado da batente baseada na orientação da porta:
                        if math.hypot(pt_porta_1[0] - parede_porta['p1'][0], pt_porta_1[1] - parede_porta['p1'][1]) < math.hypot(pt_porta_2[0] - parede_porta['p1'][0], pt_porta_2[1] - parede_porta['p1'][1]):
                            ponto_batente_oposto = pt_porta_2
                        else:
                            ponto_batente_oposto = pt_porta_1
                    ...
                    # Direção de afastamento de 15cm para fora do vão na parede
                    direcao_sinal = 1 if (ponto_batente_oposto[0] - mid_porta_x >= 0 and abs(w_vx) > 0.5) or (ponto_batente_oposto[1] - mid_porta_y >= 0 and abs(w_vy) > 0.5) else -1
                    
                    sw_x = ponto_batente_oposto[0] + (w_vx * 0.15 * direcao_sinal) + (nx * 0.12)
                    sw_y = ponto_batente_oposto[1] + (w_vy * 0.15 * direcao_sinal) + (ny * 0.12)
