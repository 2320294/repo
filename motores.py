...
def processar_dxf(caminho_arquivo):
    ...
    for polilinha in polilinhas:
        ...
        # 1. Distribuição de Luz e Interruptor Ajustado (15cm da soleira, lado oposto à dobradiça, tangenciando a parede para dentro)
        if nome in dict_dados:
            ...
            if qtd_ilum > 0:
                ...
                # POSICIONAMENTO DO INTERRUPTOR (15cm exatos da extremidade da porta/vão oposta à dobradiça)
                ...
                if unique_portas and logical_walls:
                    ...
                    # Identifica a dobradiça (ponto mais próximo da parede de apoio) e o lado oposto (maçaneta)
                    ...
                    # A extremidade do vão oposta à dobradiça serve de referência exata para medir os 15cm
                    ...
                    # Afasta exatamente 0.15m (15cm) a partir da extremidade da porta e tangencia a parede para dentro
                    sw_x = ponto_referencia_vão[0] + (w_vx * 0.15 * direcao_sinal) + (nx * 0.12)
                    sw_y = ponto_referencia_vão[1] + (w_vy * 0.15 * direcao_sinal) + (ny * 0.12)
