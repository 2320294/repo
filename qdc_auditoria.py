...
def auditar_qdc_normativo(
    ...
    # --------------------------------------------------------
    # E01 — Icc x capacidade de interrupção
    # --------------------------------------------------------
    ...
    if icc <= 0 or cap_dg <= 0 or cap_term <= 0:
        add(
            "E01",
            "Icc × capacidade de interrupção",
            "PENDENTE",
            (
                "Informar Icc no QDC, capacidade de interrupção do DG "
                "e capacidade mínima dos disjuntores terminais."
            ),
            bloqueia=False,
        )
    else:
        ...
        add(
            ...
            "OK" if ok else "ERRO",
            (
                f"Icc={icc:.1f} kA; DG={cap_dg:.1f} kA; "
                f"terminais={cap_term:.1f} kA."
            ),
            bloqueia=not ok,
        )

    # --------------------------------------------------------
    # E02 — seletividade / coordenação por referência
    # --------------------------------------------------------
    fabricante = _texto(
        parametros.get(
            "fabricante_protecao"
    ...
    if fabricante and ref_sel and sel_ok_rt:
        add(
            "E02",
            "Seletividade / coordenação",
            "VALIDADO RT",
            (
                f"{fabricante}; referência: {ref_sel}. "
                "Validação declarada pelo responsável técnico."
            ),
            bloqueia=False,
        )
    else:
        add(
            ...
            "PENDENTE",
            (
                "Informar fabricante/linha, referência de curva/tabela e "
                "confirmar a validação pelo responsável técnico."
            ),
            bloqueia=False,
        )
    ...
    if not dados_dps_completos:
        add(
            "E03",
            "Características do DPS",
            "PENDENTE",
            "Informar tipo/classe, Uc, Up, In e Imax.",
            bloqueia=False,
        )
        ...
        add(
            ...
            "ERRO",
            (
                f"{dps_tipo}: Imax={dps_imax:.1f} kA "
                f"é inferior a In={dps_in:.1f} kA."
            ),
            bloqueia=True,
        )
    else:
        add(
            ...
            "DADOS COMPLETOS",
            (
                f"{dps_tipo}; Uc={dps_uc:.0f} V; Up={dps_up:.1f} kV; "
                f"In={dps_in:.1f} kA; Imax={dps_imax:.1f} kA. "
                "Compatibilidade final é cruzada com o esquema de aterramento "
                "pela validação técnica E04."
            ),
            bloqueia=False,
        )

    # --------------------------------------------------------
    # E04 — aterramento + arranjo de DPS
    # --------------------------------------------------------
    esquema = _texto(
        parametros.get(
            "esquema_aterramento"
    ...
    if (
        esquema
        and esquema != "A definir"
        and arranjo
        and arranjo_ok_rt
    ):
        add(
            "E04",
            "Aterramento × ligação do DPS",
            "VALIDADO RT",
            (
                f"Esquema {esquema}; arranjo: {arranjo}. "
                "Compatibilidade confirmada pelo responsável técnico."
            ),
            bloqueia=False,
        )
    else:
        add(
            ...
            "PENDENTE",
            (
                "Definir esquema TN-S/TN-C-S/TT/IT, descrever o arranjo "
                "do DPS e confirmar a compatibilidade técnica."
            ),
            bloqueia=False,
        )
    ...
    if ref_conc and conc_ok_rt:
        add(
            "E05",
            "Requisitos da concessionária",
            "VALIDADO RT",
            (
                f"Referência utilizada: {ref_conc}. "
                "Requisitos adicionais confirmados pelo responsável técnico."
            ),
            bloqueia=False,
        )
    else:
        add(
            ...
            "PENDENTE",
            (
                "Informar a norma/padrão técnico utilizado e confirmar "
                "os requisitos locais do padrão de entrada."
            ),
            bloqueia=False,
        )
