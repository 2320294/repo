    ...
    with st.expander("🔄 Reenviar / Substituir Arquivo DXF (Planta Base)"):
        ...
        if uploaded_file is not None:
            ...
            try:
                ...
                st.success("✅ Novo arquivo DXF processado e salvo no Supabase!")
                st.rerun()
            except Exception as e:
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            if supabase is not None:
                ...
                supabase.table("dados_projetos").insert(payload).execute()
                st.success("✅ Projeto enviado e gravado no Supabase!")
                st.rerun()
        except Exception as e:
            st.error(f"❌ Erro ao processar o arquivo DXF: {e}")

if dados_ambientes:
    # ORDENAÇÃO ALFABÉTICA POR AMBIENTE
    dados_ambientes = sorted(dados_ambientes, key=lambda x: str(x['Ambiente']).strip().lower())
    ...
    colunas_mantidas = [
        ...
        "TUGs (Qtd)", "Pot. Unit. TUG (W)", "Carga TUGs (W)", 
        "Equipamento TUE", "Qtd TUE", "Pot. Unit. TUE (W)", "Carga TUE (W)"
    ]
    ...
    linha_total = {
        ...
        "Pot. Unit. Ilum (W)": "-",
        "Carga Ilum. (W)": int(df_exibicao["Carga Ilum. (W)"].sum()),
        "TUGs (Qtd)": int(df_exibicao["TUGs (Qtd)"].sum()),
        "Pot. Unit. TUG (W)": "-",
        "Carga TUGs (W)": int(df_exibicao["Carga TUGs (W)"].sum()),
        "Equipamento TUE": "-",
        "Qtd TUE": int(df_exibicao["Qtd TUE"].sum()),
        "Pot. Unit. TUE (W)": "-",
        "Carga TUE (W)": int(df_exibicao["Carga TUE (W)"].sum())
    }
    ...
    if st.button("💾 Salvar Alterações do Projeto no Supabase", use_container_width=True):
        if supabase is not None:
            try:
                payload = {
                    "user_email": st.session_state.user_email,
    ...
    if st.button("🚀 Gerar CAD (Atualizado)", type="primary", use_container_width=True):
        if not dxf_bytes:
            st.error("❌ Nenhum arquivo DXF associado a este projeto no Supabase. Utilize o campo 'Reenviar / Substituir Arquivo DXF' acima.")
        else:
            try:
