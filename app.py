    ...
    try:
        ...
        for polilinha in polilinhas:
            ...
            if nome_ambiente in dict_dados:
                ...
                if nome_ambiente == qdc_formatado and not is_area_umida:
                    ...
                    if logical_walls:
                        ...
                        for sol in unique_soleiras:
                            ...
                            min_d = float('inf'); closest_lw = None
                            for lw in logical_walls:
                ...
                # 3. TOMADAS E TUES (FLUXO CONTÍNUO A 10CM)
                ...
                if total_tomadas > 0 and comp_total > 0:
                    ...
                    while tomadas_pos < total_tomadas and tentativas < total_tomadas * 5:
                        ...
                        perto_porta = False
                        if hinge and latch and (math.hypot(px - hinge[0], py - hinge[1]) < 0.8 or math.hypot(px - latch[0], py - latch[1]) < 0.5): perto_porta = True
                        for sol in unique_soleiras:
                            if math.hypot(px - (sol['p1'][0]+sol['p2'][0])/2, py - (sol['p1'][1]+sol['p2'][1])/2) < 0.6: perto_porta = True; break
                        if ac_placed and math.hypot(px - mx_ac, py - my_ac) < 0.6: perto_porta = True
                        
                        if perto_porta: dist_atual += (0.4 * dir_step); continue
                        ...
                        if math.hypot(centro_x - ponta1[0], centro_y - ponta1[1]) < math.hypot(centro_x - ponta2[0], centro_y - ponta2[1]):
                            ux_n, uy_n, pt_ponta = n1x, n1y, ponta1
                        else: ux_n, uy_n, pt_ponta = n2x, n2y, ponta2
                        
                        pt_base1 = (px + ux_w * 0.15, py + uy_w * 0.15)
                        pt_base2 = (px - ux_w * 0.15, py - uy_w * 0.15)
                        
                        is_tue = tomadas_pos >= qtd_tugs
                        fill_mode = "full" if is_tue else ("half" if is_area_umida else "empty")
                        
                        msp.add_lwpolyline([pt_base1, pt_base2, pt_ponta, pt_base1], close=True, dxfattribs={'layer': 'PROJ_ELETRICA_TOMADA'})
                        if fill_mode == "full": msp.add_solid([pt_base1, pt_base2, pt_ponta], dxfattribs={'layer': 'PROJ_ELETRICA_TOMADA'})
                        elif fill_mode == "half": msp.add_solid([pt_base1, (px, py), pt_ponta], dxfattribs={'layer': 'PROJ_ELETRICA_TOMADA'})
                        
                        if is_tue:
                            pot_tue_val = int(dados_amb.get('Pot. Unit. TUE (VA)', 0))
                            if pot_tue_val > 0: msp.add_text(f"{pot_tue_val}W", dxfattribs={'layer': 'PROJ_ELETRICA_TEXTO', 'height': 0.1, 'color': 4, 'insert': (px + ux_n * 0.4, py + uy_n * 0.4)})
...
def tela_login():
    ...
    if supabase is None: st.error("Erro Crítico: Supabase não conectado."); st.stop()
            ...
            with st.form("form_login"):
                ...
                if st.form_submit_button("Entrar no Sistema"):
                    try:
                        ...
                        st.success("Login com sucesso!"); st.rerun()
                    except: st.error("Credenciais inválidas.")
        with tab_cadastro:
            with st.form("form_cadastro"):
                ne = st.text_input("Seu E-mail"); ns = st.text_input("Senha (mín. 6)", type="password")
                if st.form_submit_button("Criar Conta"):
                    if len(ns) < 6: st.warning("Senha curta.")
                    else:
                        try: supabase.auth.sign_up({"email": ne, "password": ns}); st.success("Conta criada! Verifique o e-mail.")
...
def sistema_principal():
    with st.sidebar:
        ...
        obras_usuario = supabase.table("obras").select("*").eq("user_id", st.session_state.user_id).execute().data
        with st.expander("➕ Novo Projeto / Pavimento"):
            no = st.text_input("Nome da Obra", placeholder="Ex: Casa Praia"); np = st.text_input("Pavimento", placeholder="Ex: Térreo")
            if st.button("Criar e Salvar"):
                if no and np: supabase.table("obras").insert({"user_id": st.session_state.user_id, "nome_obra": no, "pavimento": np, "dados_json": []}).execute(); st.success("Criado!"); st.rerun()
        if obras_usuario:
            opcoes = {f"{ob['nome_obra']} - {ob['pavimento']}": ob for ob in obras_usuario}
            escolha = st.selectbox("Selecione a obra:", ["Nenhum"] + list(opcoes.keys()))
            if escolha != "Nenhum":
                sel = opcoes[escolha]
                if "obra_atual" not in st.session_state or st.session_state.obra_atual['id'] != sel['id']:
                    st.session_state.obra_atual = sel; st.session_state.dados_extraidos = sel.get("dados_json", []); st.rerun()
                st.divider()
                if st.button("🗑️ Excluir Pavimento", type="primary", use_container_width=True):
                    supabase.table("obras").delete().eq("id", st.session_state.obra_atual['id']).execute()
                    st.session_state.obra_atual = None; st.session_state.dados_extraidos = None; st.rerun()
            else: st.session_state.obra_atual = None; st.session_state.dados_extraidos = None
    ...
    if "obra_atual" not in st.session_state or st.session_state.obra_atual is None:
        st.info("👈 Crie ou selecione um projeto no menu lateral."); return

    st.subheader(f"🏢 {st.session_state.obra_atual['nome_obra']} | 📍 {st.session_state.obra_atual['pavimento']}")
    st.divider()

    if not st.session_state.dados_extraidos:
        st.write("### 1. Importação da Planta Baixa (DXF)")
        arq = st.file_uploader("Arquivo .dxf", type=["dxf"])
        if arq and st.button("Ler Arquivo CAD", type="primary"):
            with tempfile.NamedTemporaryFile(delete=False, suffix=".dxf") as tmp:
                tmp.write(arq.getvalue()); tmp_p = tmp.name
            try:
                res = processar_dxf(tmp_p)
                if res:
                ...
                else: st.warning("Nenhum ambiente encontrado.")
            finally: os.remove(tmp_p)
    else:
        st.success("✅ Planta carregada!")
        df_base = pd.DataFrame(st.session_state.dados_extraidos)
        amb_seg = [a for a in df_base['Ambiente'].tolist() if not any(x in a.lower() for x in ["coz", "serv", "banh", "lav", "wc", "bwc", "sanit"])]
        op_qdc = ["Selecione o ambiente..."] + [(f"{a} (recomendado)" if any(t in a.lower() for t in ["hall", "corredor", "circulação"]) else a) for a in amb_seg]
        
        c1, c2, c3 = st.columns(3)
        with c1: tensao = st.radio("Tensão:", [127, 220], horizontal=True)
        with c2: pe_dir = st.number_input("Pé Direito:", value=2.80)
        with c3: qdc_sel = st.selectbox("Locação do QDC:", options=list(dict.fromkeys(op_qdc)))
        
        df_ed = df_base.copy()
        with st.expander("✏️ Ajustar Cargas", expanded=True):
            for i, r in df_ed.iterrows():
                col1, col2, col3 = st.columns(3)
                with col1: st.write(f"**{r['Ambiente']}**")
                with col2: df_ed.at[i, 'Qtd Ilum.'] = st.number_input("Ilum", value=int(r['Qtd Ilum.']), key=f"ilum_{i}")
                with col3: df_ed.at[i, 'TUGs (Qtd)'] = st.number_input("TUGs", value=int(r['TUGs (Qtd)']), key=f"tugs_{i}")
                st.divider()

        if st.button("💾 Salvar na Nuvem", type="primary"):
            supabase.table("obras").update({"dados_json": df_ed.to_dict('records'), "local_qdc": qdc_sel, "tensao_projeto": int(tensao), "pe_direito": float(pe_dir)}).eq("id", st.session_state.obra_atual['id']).execute()
            st.session_state.dados_extraidos = df_ed.to_dict('records'); st.success("Salvo!")

        st.write("### 🖨️ Exportação CAD Unifilar")
        arq_base = st.file_uploader("Reenvie o .dxf para gerar o desenho:", type=["dxf"], key="dxf_uni")
        if arq_base and st.button("🎨 Gerar DXF Final (Motor 34.2)", type="primary"):
            dxf_bytes = gerar_cad_unifilar(arq_base.getvalue(), df_ed.to_dict('records'), qdc_sel)
            st.download_button("⬇️ Baixar DXF Desenhado", data=dxf_bytes, file_name=f"Projeto_{st.session_state.obra_atual['nome_obra']}.dxf", mime="application/dxf", use_container_width=True)
