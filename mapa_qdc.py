...
# ============================================================
# FASE 13.6 REV.40 — PASSAGENS "POR TRÁS" DOS DISJUNTORES
# ============================================================
_QDC_DJ_RECTS = []
...
def _line(msp, p1, p2, layer):
    if layer in _QDC_LAYERS_CONDUTORES:
        criadas = []
        for a, b in _segmentos_fora_dos_disjuntores(p1, p2):
            if (
...
def _desenhar_no_confirmado(msp, x, y, token, raio=0.045):
...
def _finalizar_nos_topologicos(msp):
    ...
    bolinha somente quando o ponto tiver 3 ou mais ramos reais.
    """
    vistos = set()
...
def _desenhar_dispositivo(
    ...
    #
    # Rev.40 — cada entrada e saída física de cabo recebe também
    # um pequeno retângulo de conexão exatamente na borda do aparelho.
    # Dimensões definidas:
    #   largura = 0,05
    #   altura  = 0,075
    #
    # O retângulo fica centrado no eixo do polo e centrado sobre
    # a própria face superior/inferior do dispositivo.
    ...
    for i in range(modulos):
        ...
        # Terminal superior — entrada de cabo.
        _rect(
            msp,
            cx - terminal_largura / 2.0,
            y2 - terminal_altura / 2.0,
            cx + terminal_largura / 2.0,
            y2 + terminal_altura / 2.0,
            layer
        )

        # Terminal inferior — saída de cabo.
        _rect(
            msp,
            cx - terminal_largura / 2.0,
            y1 - terminal_altura / 2.0,
            cx + terminal_largura / 2.0,
            y1 + terminal_altura / 2.0,
            layer
        )
...
def desenhar_mapa_fisico_qdc(
    ...
    """
    Fase 13.6 Rev.40 — QDC executivo no CAD.
    ...
    # Fase 13.6 Rev.40 — padrão modular do QDC.
    ...
    _text(
        msp,
        "VISTA FRONTAL - DIAGRAMA DE MONTAGEM E LIGACOES | FASE 13.6 REV.40",
        x0 + 0.55,
        y0 - 0.92,
        0.11,
        LT
    ...
    # Fase 13.6 Rev.40 — eixo geométrico único do "miolo" do QDC.
    ...
    # Fase 13.6 Rev.40 — grade vertical equidistante das seis linhas
    ...
    if dg_geoms:
        ...
        # ====================================================
        # FASE 13.6 REV.40 — ENTRADA DA REDE
        # ====================================================
    ...
    if geral_geom:
        ...
        # ----------------------------------------------------
        # FASE 13.6 REV.40 — CONVENÇÃO DE NÓS DE DERIVAÇÃO
        # ----------------------------------------------------
    ...
    # ========================================================
    # FASE 13.6 REV.40 — NEUTRO DOS IDRs PELO 2º BORNE
    # ========================================================
    # Regras:
    ...
    for trilho in range(trilhos_circuitos):
        ...
        # Fase 13.6 Rev.40 — SAÍDAS DOS CIRCUITOS
        # ------------------------------------------------------------
        ...
        if circuitos_geom:
            ...
            if desta_fileira:
                ...
                # ====================================================
                # Fase 13.6 Rev.40 — GRADE VERTICAL DINÂMICA DA FILEIRA
                # ====================================================
                ...
                if (
                    ...
                    if y_pe_dps is not None:
                        ...
                        # 2º borne PE -> nível harmônico -> tronco horizontal.
                        _polyline(
                            msp,
                            [
