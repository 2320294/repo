...
def desenhar_mapa_fisico_qdc(
    ...
    """
    Fase 13.6 Rev.28 — QDC executivo no CAD.
    ...
    # Fase 13.6 Rev.28 — padrão modular do QDC.
    ...
    _text(
        msp,
        "VISTA FRONTAL - DIAGRAMA DE MONTAGEM E LIGACOES | FASE 13.6 REV.28",
        x0 + 0.55,
        y0 - 0.92,
        0.11,
        LT
    ...
    # Fase 13.6 Rev.28 — eixo geométrico único do "miolo" do QDC.
    ...
    # Fase 13.6 Rev.28 — grade vertical equidistante das seis linhas
    ...
    if dg_geoms:
        ...
        # ====================================================
        # FASE 13.6 REV.28 — ENTRADA DA REDE
        # ====================================================
    ...
    if geral_geom:
        ...
        # ----------------------------------------------------
        # FASE 13.6 REV.28 — CONVENÇÃO DE NÓS DE DERIVAÇÃO
        # ----------------------------------------------------
    ...
    # ========================================================
    # FASE 13.6 REV.28 — NEUTRO DOS IDRs PELO 2º BORNE
    # ========================================================
    # Regras:
    ...
    for trilho in range(trilhos_circuitos):
        ...
        # Fase 13.6 Rev.28 — SAÍDAS DOS CIRCUITOS
        # ------------------------------------------------------------
        ...
        if circuitos_geom:
            ...
            if desta_fileira:
                ...
                # ====================================================
                # Fase 13.6 Rev.28 — GRADE VERTICAL DINÂMICA DA FILEIRA
                # ====================================================
                    ...
                    if fonte_geom and fonte_disp:
                        ...
                        if usar_pente:
                            ...
                            for idx_fase, fase_item in enumerate(
                                ...
                                # Rev.27 — REGRA FIXA DO QDC:
                                # todo condutor sai RETO NA VERTICAL do borne.
                                # A primeira mudança de direção só ocorre no
                                # nível horizontal reservado, longe do aparelho.
                                # Rev.28 — ajuste pontual de legibilidade:
                                # a fase A proveniente do DR3 usa corredor
                                # vertical 0,105 à esquerda para não coincidir
                                # visualmente com o eixo do C01/DG.
                                x_vertical = x_origem
                                if (
                                ...
                                if abs(x_vertical - x_origem) > 1e-9:
                                    # Sai reto do borne; o deslocamento ocorre
                                    # no nível horizontal de distribuição.
                                    _polyline(
                                        msp,
                                        [
