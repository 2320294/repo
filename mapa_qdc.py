...
# ============================================================
# FASE 13.6 REV.70 — PASSAGENS "POR TRÁS" DE TODOS OS APARELHOS
# ============================================================
_QDC_DJ_RECTS = []
...
def _aplicar_desvio_especifico_c01_fase_a_rev70(
    msp,
    circuitos_geom,
    deslocamento=0.10,
):
    """
    Rev.70 — correção visual pontual.

    Se existir C01 na fileira imediatamente acima de C09 e a fase A de C01
    estiver no mesmo eixo X da fase A de C09, desloca somente o trecho de
    passagem entre as duas fileiras em 0,10 para a esquerda.

    A conexão de C01 continua correta acima do vão e a de C09 continua
    reta no próprio borne. O objetivo é evitar sobreposição visual.
    """
    mapa = {
    ...
    fases09 = _fases_do_texto(d09.get("fase", ""))

    if "A" not in fases01 or "A" not in fases09:
        return

    x01 = float(_polo_para_fase(d01, g01, "A"))
    x09 = float(_polo_para_fase(d09, g09, "A"))

    # Só aplica se realmente houver coincidência de eixo.
    if abs(x01 - x09) > 1e-6:
        return

    # C01 precisa estar acima de C09.
    ...
    y_base_c09 = float(g09["y1"])

    x_desvio = x01 - float(deslocamento)

    # Remove o trecho vertical de passagem da fase A de C01 que cruza
    # o intervalo até C09, sem mexer em outras fases/circuitos.
    entidades = list(msp)
    for ent in entidades:
        try:
            if ent.dxftype() != "LINE":
            ...
            # Segmento que cobre o vão superior até/através de C09.
            if yhi >= y_inicio - 1e-6 and ylo <= y_base_c09 + 1e-6:
                try:
    ...
    # Recria a rota:
    # sai de C01, desloca 0,10 para a esquerda antes de C09,
    # desce por trás do C09 e volta ao eixo original abaixo dele.
    y_acima_c09 = y_topo_c09 + 0.10
    y_abaixo_c09 = y_base_c09 - 0.10
    ...
    _polyline(
        ...
        [
            (x01, y_inicio),
            (x01, y_acima_c09),
            (x_desvio, y_acima_c09),
            (x_desvio, y_abaixo_c09),
            (x01, y_abaixo_c09),
        ],
        layer_a
    )
...
def desenhar_mapa_fisico_qdc(
    ...
    """
    Fase 13.6 Rev.70 — QDC executivo no CAD.
    ...
    # Fase 13.6 Rev.70 — padrão modular do QDC.
    ...
    _text(
        msp,
        "VISTA FRONTAL - DIAGRAMA DE MONTAGEM E LIGACOES | FASE 13.6 REV.70",
        x0 + 0.55,
        y0 - 0.92,
        0.11,
        LT
    ...
    # Fase 13.6 Rev.70 — eixo geométrico único do "miolo" do QDC.
    ...
    # Fase 13.6 Rev.70 — grade vertical equidistante das seis linhas
    ...
    if dg_geoms:
        ...
        # ====================================================
        # FASE 13.6 REV.70 — ENTRADA DA REDE
        # ====================================================
    ...
    if geral_geom:
        ...
        # ----------------------------------------------------
        # FASE 13.6 REV.70 — CONVENÇÃO DE NÓS DE DERIVAÇÃO
        # ----------------------------------------------------
    ...
    # ========================================================
    # FASE 13.6 REV.70 — NEUTRO DOS IDRs PELO 2º BORNE
    # ========================================================
    # Regras:
    ...
    for trilho in range(trilhos_circuitos):
        ...
        # Fase 13.6 Rev.70 — SAÍDAS DOS CIRCUITOS
        # ------------------------------------------------------------
        ...
        if circuitos_geom:
            ...
            if desta_fileira:
                ...
                # ====================================================
                # Fase 13.6 Rev.70 — GRADE VERTICAL DINÂMICA DA FILEIRA
                # ====================================================
    ...
    # ========================================================
    # FASE 13.6 REV.70 — NEUTROS PELA DIREITA, POR FONTE
    # ========================================================
    ...
    # ========================================================
    # FASE 13.6 REV.70 — PE INDIVIDUAL POR CIRCUITO
    # ========================================================
    ...
    # ========================================================
    # FASE 13.6 REV.70 — CHICOTES FINAIS AGRUPADOS POR CIRCUITO
    # ========================================================
    # Regras visuais:
    ...
    # Rev.70 — ajuste pontual da fase A de C01 quando coincide
    # com a entrada de fase A de C09.
    _aplicar_desvio_especifico_c01_fase_a_rev70(
        msp,
        circuitos_geom,
        0.10
    )
